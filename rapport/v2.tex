...
\Input{ TDB $\mathcal{T}$, tabu tenure \emph{tt}, total time \emph{TT}, profondeur $\beta$, nombre maximal de TS \emph{MaxTS} }
...
\SetKwInOut{Output}{Output}
\Input{ TDB $\mathcal{T}$, tabu tenure \emph{tt}, total time \emph{TT}, nombre max sans amélioration $\beta$ }
\Output{ Solution \emph{$S_{b}$} }
\Begin{
	
	Solution \emph{S} = randomInit(); \\
	Solution \emph{$S_{b}$} = \emph{S}; \\
	d = 0; iteration = 0; \\
	\Repeat{ d == $\beta$ || timeLeft == 0}{
		Solution \emph{S'} = BestNeighboor(\emph{S}); \\
		MaJTabuList(\emph{S'}, \emph{S}, $\emph{tt}+iteration$); \\
		\emph{S} $\longleftarrow$ \emph{S'}; \\
		
		\If{ Evaluation(\emph{S'}) $>$ Evaluation($\emph{S}_{b}$) }{
